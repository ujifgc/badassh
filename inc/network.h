...
struct plug_function_table {
	void (*log)(Plug p, int type, SockAddr addr, int port, const char *error_msg, int error_code);
	/*
	 ...
	 */
	int (*closing)(Plug p, const char *error_msg, int error_code, int calling_back);
...
Socket new_listener(char *srcaddr, int port, Plug plug, int local_host_only, const Config *cfg, int addressfamily);
...
Socket sk_new(SockAddr addr, int port, int privport, int oobinline, int nodelay, int keepalive, Plug p);
...
#define plug_dns(p, addr) (((*p)->dns)(p, addr))
#endif
