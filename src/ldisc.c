...
static void __pwrite(Ldisc ldisc, unsigned char c) {
	if ((c >= 32 && c <= 126) || (!in_utf(ldisc->term) && c >= 0xA0) || (in_utf(ldisc->term) && c >= 0x80)) {
...
void *ldisc_create(Config *mycfg, Terminal *term, Backend *back, void *backhandle, void *frontend) {
	...
	if (EDITING) {
		while (len--) {
			...
			case CTRL('W'): /* delete word */
				while (ldisc->buflen > 0) {
					if (ECHOING)
						bsb(ldisc, plen(ldisc, ldisc->buf[ldisc->buflen - 1]));
					ldisc->buflen--;
					if (ldisc->buflen > 0 && isspace((unsigned char) ldisc->buf[ldisc->buflen - 1]) && !isspace(
			...
			case CTRL('D'): /* logout or send */
				 ...
				 */
			case CTRL('J'):
				if (ldisc->cfg->protocol == PROT_RAW && ldisc->buflen > 0 && ldisc->buf[ldisc->buflen - 1] == '\r') {
					if (ECHOING)
						bsb(ldisc, plen(ldisc, ldisc->buf[ldisc->buflen - 1]));
					ldisc->buflen--;
					/* FALLTHROUGH */
			...
			default: /* get to this label from ^V handler */
				...
				if (ECHOING)
					__pwrite(ldisc, (unsigned char) c);
