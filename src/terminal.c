...
void do_paint(Terminal *, Context, int);
 ...
 * fields to be.
 */
static int termchars_equal_override(termchar *a, termchar *b, unsigned long bchr, unsigned long battr) {
	/* FULL-TERMCHAR */
	if (a->chr != bchr)
		return FALSE;
...
static void makerle(struct buf *b, termline *ldata,
	...
	while (n-- > 0) {
		...
		if (thislen == prevlen && !memcmp(b->data + prevpos, b->data + thispos, thislen)) {
			/*
			...
			if (thislen > 1 || prev2) {
				...
				while (n > 0 && runlen < 129) {
					...
					tmplen = b->len - tmppos;
					b->len = tmppos;
					if (tmplen != thislen || memcmp(b->data + runpos + 1, b->data + tmppos, tmplen)) {
						state = oldstate;
						break; /* run over */
					}
					n--, c++, runlen++;
...
static void makeliteral_attr(struct buf *b, termchar *c, unsigned long *state) {
	...
	attr = (((attr >> (ATTR_BGSHIFT + 8)) << (ATTR_BGSHIFT + 4)) | (attr & ((1 << (ATTR_BGSHIFT + 4)) - 1)));
	attr = (((attr >> (ATTR_FGSHIFT + 8)) << (ATTR_FGSHIFT + 4)) | (attr & ((1 << (ATTR_FGSHIFT + 4)) - 1)));
...
static void readrle(struct buf *b, termline *ldata, void(*readliteral)(struct buf *b, termchar *c, termline *ldata,
		unsigned long *state)) {
	int n = 0;
...
static void readliteral_chr(struct buf *b, termchar *c, termline *ldata, unsigned long *state) {
	int byte;
...
static void readliteral_attr(struct buf *b, termchar *c, termline *ldata, unsigned long *state) {
	...
	attr = (((attr >> (ATTR_FGSHIFT + 4)) << (ATTR_FGSHIFT + 8)) | (attr & ((1 << (ATTR_FGSHIFT + 4)) - 1)));
	attr = (((attr >> (ATTR_BGSHIFT + 4)) << (ATTR_BGSHIFT + 8)) | (attr & ((1 << (ATTR_BGSHIFT + 4)) - 1)));
...
static void readliteral_cc(struct buf *b, termchar *c, termline *ldata, unsigned long *state) {
	termchar n;
	unsigned long zstate;
	int x = c - ldata->chars;
...
static void resizeline(Terminal *term, termline *line, int cols) {
	...
	if (line->cols != cols) {
		 ...
		 */
		if (cols < oldcols)
			memmove(line->chars + cols, line->chars + oldcols, (line->size - line->cols) * TSIZE);
...
static termline *lineptr(Terminal *term, int y, int lineno, int screen) {
	...
	if (line == NULL) {
		fatalbox("line==NULL in terminal.c\n"
			...
			"and pass on the above information.", lineno, y, term->cols, term->rows, term->scrollback, count234(
				term->scrollback), term->screen, count234(term->screen), term->alt_screen, count234(term->alt_screen),
 ...
 */
static void term_schedule_vbell(Terminal *term, int already_started, long startpoint) {
	...
	if (ticks_already_gone < VBELL_DELAY) {
		term->in_vbell = TRUE;
		term->vbell_end = schedule_timer(VBELL_DELAY - ticks_already_gone, term_timer, term);
	} else {
		term->in_vbell = FALSE;
	}
}
...
static void power_on(Terminal *term, int clear) {
	...
	term->alt_wnext = term->wrapnext = term->save_wnext = term->alt_save_wnext = FALSE;
	...
	term->utf_state = 0;
	term->alt_sco_acs = term->sco_acs = term->save_sco_acs = term->alt_save_sco_acs = 0;
	term->cset_attr[0] = term->cset_attr[1] = term->save_csattr = term->alt_save_csattr = CSET_ASCII;
	term->rvideo = 0;
	term->in_vbell = FALSE;
	term->cursor_on = 1;
	term->big_cursor = 0;
	term->default_attr = term->save_attr = term->alt_save_attr = term->curr_attr = ATTR_DEFAULT;
...
static void update_sbar(Terminal *term) {
	...
	set_sbar(term->frontend, nscroll + term->rows, nscroll + term->disptop, term->rows);
}
...
static void insch(Terminal *term, int n) {
	...
	if (dir < 0) {
		for (j = 0; j < m; j++)
			move_termchar(ldata, ldata->chars + term->curs.x + j, ldata->chars + term->curs.x + j + n);
		while (n--)
			copy_termchar(ldata, term->curs.x + m++, &term->erase_char);
	} else {
		for (j = m; j--;)
			move_termchar(ldata, ldata->chars + term->curs.x + j + n, ldata->chars + term->curs.x + j);
		while (n--)
...
static void do_osc(Terminal *term) {
	if (term->osc_w) {
		while (term->osc_strlen--)
			term->wordness[(unsigned char) term->osc_string[term->osc_strlen]] = term->esc_args[0];
	} else {
	...
	while (nchars > 0 || unget != -1 || bufchain_size(&term->inbuf) > 0) {
		...
		if (term->termstate == TOPLEVEL) {
			if (in_utf(term))
				...
				case 5:
					...
					if (c < 0x80 || (c < 0x800 && term->utf_size >= 2) || (c < 0x10000 && term->utf_size >= 3) || (c
							< 0x200000 && term->utf_size >= 4) || (c < 0x4000000 && term->utf_size >= 5))
						c = UCSERR;
		...
		if ((c & ~0x1F) == 0 && term->termstate < DO_CTRLS) {
			...
			case '\007': /* BEL: Bell */
			{
				 ...
				 * t seconds ago.
				 */
				while (term->beephead && term->beephead->ticks < ticks - term->cfg.bellovl_t) {
					struct beeptime *tmp = term->beephead;
					term->beephead = tmp->next;
					sfree(tmp);
					if (!term->beephead)
				...
				if (term->cfg.bellovl && term->beep_overloaded && ticks - term->lastbeep
					 ...
					 */
					term->beep_overloaded = FALSE;
				} else if (term->cfg.bellovl && !term->beep_overloaded && term->nbeeps >= term->cfg.bellovl_n) {
					/*
				 ...
				 * ctrls are stripped above */
			{
				...
				if (DIRECT_CHAR(c))
					width = 1;
				if (!width)
					width = (term->cfg.cjk_ambig_wide ? mk_wcwidth_cjk((wchar_t) c) : mk_wcwidth((wchar_t) c));
			...
			case SEEN_ESC:
				...
				case 'Z': /* DECID: terminal type query */
					...
					;
					if (term->ldisc)
						ldisc_send(term->ldisc, term->id_string, strlen(term->id_string), 0);
					break;
			...
			case SEEN_CSI:
				...
				if (isdigit(c)) {
					...
					case 'A': /* CUU: move up N lines */
						move(term, term->curs.x, term->curs.y - def(term->esc_args[0], 1), 1);
						seen_disp_event(term);
						break;
					...
					case 'C': /* CUF: Cursor right */
						move(term, term->curs.x + def(term->esc_args[0], 1), term->curs.y, 1);
						seen_disp_event(term);
						break;
					...
					case 'd': /* VPA: set vertical posn */
						compatibility(ANSI)
						;
						move(term, term->curs.x, ((term->dec_om ? term->marg_t : 0) + def(term->esc_args[0], 1) - 1),
								(term->dec_om ? 2 : 0));
						seen_disp_event(term);
						break;
					case 'H': /* CUP */
					case 'f': /* HVP: set horz and vert posns at once */
						...
						move(term, def(term->esc_args[1], 1) - 1, ((term->dec_om ? term->marg_t : 0)
					...
					case 'L': /* IL: insert lines */
						...
						;
						if (term->curs.y <= term->marg_b)
							scroll(term, term->curs.y, term->marg_b, -def(term->esc_args[0], 1), FALSE);
						seen_disp_event(term);
						break;
					case 'M': /* DL: delete lines */
						...
						;
						if (term->curs.y <= term->marg_b)
							scroll(term, term->curs.y, term->marg_b, def(term->esc_args[0], 1), TRUE);
						seen_disp_event(term);
						break;
					...
					case 't': /* DECSLPP: set page size - ie window height */
						 ...
						 * and reports.
						 */
						if (term->esc_nargs <= 1 && (term->esc_args[0] < 1 || term->esc_args[0] >= 24)) {
							compatibility(VT340TEXT);
							if (!term->cfg.no_remote_resize)
								request_resize(term->frontend, term->cols, def(term->esc_args[0], 24));
							deselect(term);
						} else if (term->esc_nargs >= 1 && term->esc_args[0] >= 1 && term->esc_args[0] < 24) {
							compatibility(OTHER);
						...
						case 3:
							if (term->esc_nargs >= 3) {
								if (!term->cfg.no_remote_resize)
									move_window(term->frontend, def(term->esc_args[1], 0), def(term->esc_args[2], 0));
							}
							break;
						case 4:
						...
						case 8:
							if (term->esc_nargs >= 3) {
								if (!term->cfg.no_remote_resize)
									request_resize(term->frontend, def(term->esc_args[2], term->cfg.width),
						...
						case 11:
							if (term->ldisc)
								ldisc_send(term->ldisc, is_iconic(term->frontend) ? "\033[1t" : "\033[2t", 4, 0);
							break;
						case 13:
							if (term->ldisc) {
						...
						case 19:
							...
							break;
						case 20:
							if (term->ldisc && term->cfg.remote_qtitle_action != TITLE_NONE) {
					...
					case 'S': /* SU: Scroll up */
						compatibility(SCOANSI)
						;
						scroll(term, term->marg_t, term->marg_b, def(term->esc_args[0], 1), TRUE);
						term->wrapnext = FALSE;
						seen_disp_event(term);
						break;
					...
					case ANSI('|', '*'): /* DECSNLS */
						...
						if (term->esc_nargs == 1 && term->esc_args[0] > 0) {
							if (!term->cfg.no_remote_resize)
								request_resize(term->frontend, term->cols, def(term->esc_args[0],
										term->cfg.height));
							deselect(term);
						}
						break;
					case ANSI('|', '$'): /* DECSCPP */
						...
						if (term->esc_nargs <= 1) {
							if (!term->cfg.no_remote_resize)
								request_resize(term->frontend, def(term->esc_args[0],
					...
					case ANSI('F', '='): /* set normal foreground */
						...
						if (term->esc_args[0] >= 0 && term->esc_args[0] < 16) {
							long colour = (sco2ansicolour[term->esc_args[0] & 0x7] | (term->esc_args[0] & 0x8))
									<< ATTR_FGSHIFT;
							term->curr_attr &= ~ATTR_FGMASK;
			...
			case SEEN_OSC_P: {
				...
				if (term->osc_strlen >= 7) {
					palette_set(term->frontend, term->osc_string[0], term->osc_string[1] * 16 + term->osc_string[2],
							term->osc_string[3] * 16 + term->osc_string[4], term->osc_string[5] * 16
									+ term->osc_string[6]);
					term_invalidate(term);
					term->termstate = TOPLEVEL;
				}
 ...
 */
static inline termchar *term_bidi_line(Terminal *term, struct termline *ldata, int scr_y) {
	return NULL;
}
 ...
 */
void do_paint(Terminal *term, Context ctx, int may_optimise) {
	 ...
	 * previous position.
	 */
	if (term->dispcursy >= 0 && (term->curstype != cursor || term->dispcursy != our_curs_y || term->dispcursx
			!= our_curs_x)) {
	...
	for (i = 0; i < term->rows; i++) {
		...
		for (j = 0; j < term->cols; j++) {
			...
			if (!term->cfg.ansi_colour)
				tattr = (tattr & ~(ATTR_FGMASK | ATTR_BGMASK)) | ATTR_DEFFG | ATTR_DEFBG;
			...
			if (term->selstate == DRAGGING || term->selstate == SELECTED) {
				if (term->seltype == LEXICOGRAPHIC)
					selected = (posle(term->selstart, scrpos) && poslt(scrpos, term->selend));
				else
					selected = (posPle(term->selstart, scrpos) && posPlt(scrpos, term->selend));
			} else
				selected = FALSE;
			 ...
			 */
			if (tchar != term->disptext[i]->chars[j].chr || tattr != (term->disptext[i]->chars[j].attr & ~(ATTR_NARROW
					| DATTR_MASK))) {
		...
		for (j = 0; j < term->cols; j++) {
			...
			if (term->disptext[i]->chars[j].chr != newline[j].chr || (term->disptext[i]->chars[j].attr & ~DATTR_MASK)
					!= newline[j].attr) {
				int k;
		...
		for (j = 0; j < term->cols; j++) {
			...
			if (!term->ucsdata->dbcs_screenfont && !dirty_line) {
				if (term->disptext[i]->chars[j].chr == tchar && (term->disptext[i]->chars[j].attr & ~DATTR_MASK)
						== tattr)
					break_run = TRUE;
			...
			do_copy = FALSE;
			if (!termchars_equal_override(&term->disptext[i]->chars[j], d, tchar, tattr)) {
				do_copy = TRUE;
				dirty_run = TRUE;
			}
...
static void clipme(Terminal *term, pos top, pos bottom, int rect, int desel) {
	...
	while (poslt(top, bottom)) {
		...
		if (!(ldata->lattr & LATTR_WRAPPED)) {
			while (nlpos.x && IS_SPACE_CHR(ldata->chars[nlpos.x - 1].chr) && !ldata->chars[nlpos.x - 1].cc_next
					&& poslt(top, nlpos))
				decpos(nlpos);
...
static int wordtype(Terminal *term, int uc) {
	...
	static const struct ucsword ucs_words[] = { { 128, 160, 0 }, { 161, 191, 1 }, { 215, 215, 1 }, { 247, 247, 1 }, {
	 ...
	 */
	if (term->ucsdata->dbcs_screenfont && term->ucsdata->font_codepage == term->ucsdata->line_codepage)
		return (uc != ' ');
...
static pos sel_spread_half(Terminal *term, pos p, int dir) {
	...
	case SM_WORD:
		...
		if (dir == +1) {
			while (1) {
				int maxcols = (ldata->lattr & LATTR_WRAPPED2 ? term->cols - 1 : term->cols);
				if (p.x < maxcols - 1) {
			...
			while (1) {
				if (p.x > 0) {
					...
					if (p.y <= topy)
						break;
					ldata2 = lineptr(p.y-1);
					maxcols = (ldata2->lattr & LATTR_WRAPPED2 ? term->cols - 1 : term->cols);
...
void term_do_paste(Terminal *term) {
	...
	if (data && len > 0) {
		...
		p = q = data;
		while (p < data + len) {
			while (p < data + len && !(p <= data + len - sel_nl_sz && !memcmp(p, sel_nl, sizeof(sel_nl))))
				p++;
...
void term_mouse(Terminal *term, Mouse_Button braw, Mouse_Button bcooked, Mouse_Action a, int x, int y, int shift,
		int ctrl, int alt) {
	pos selpoint;
	termline *ldata;
	int raw_mouse = (term->xterm_mouse && !term->cfg.no_mouse_rep && !(term->cfg.mouse_override && shift));
	int default_seltype;
	 ...
	 * presses.
	 */
	if (raw_mouse && (term->selstate != ABOUT_TO) && (term->selstate != DRAGGING)) {
		int encstate = 0, r, c;
		char abuf[16];
	...
	if (bcooked == MBT_SELECT && a == MA_CLICK) {
		...
		incpos(term->selend);
		sel_spread(term);
	} else if ((bcooked == MBT_SELECT && a == MA_DRAG) || (bcooked == MBT_EXTEND && a != MA_RELEASE)) {
		...
		if (bcooked == MBT_EXTEND && a != MA_DRAG && term->selstate == SELECTED) {
			if (term->seltype == LEXICOGRAPHIC) {
				 ...
				 * to the mouse.
				 */
				if (posdiff(selpoint, term->selstart) < posdiff(term->selend, term->selstart) / 2) {
		...
		sel_spread(term);
	} else if ((bcooked == MBT_SELECT || bcooked == MBT_EXTEND) && a == MA_RELEASE) {
...
void term_key(Terminal *term, Key_Sym keysym, wchar_t *text, size_t tlen, unsigned int modifiers, unsigned int flags) {
	char output[10];
	char *p = output;
	...
	if (keysym == PK_NULL && (modifiers & PKM_CONTROL) && tlen == 1 && text[0] >= 0x20 && text[0] <= 0x7e) {
		/* ASCII chars + Control */
		if ((text[0] >= 0x40 && text[0] <= 0x5f) || (text[0] >= 0x61 && text[0] <= 0x7a))
			text[0] &= 0x1f;
		else {
			/*
	...
	if (PK_ISKEYPAD(keysym)) {
		 ...
		 */
		if (term->cfg.funky_type == FUNKY_VT400 || ((term->cfg.funky_type == FUNKY_LINUX || term->cfg.funky_type
				== FUNKY_TILDE) && term->app_keypad_keys && !term->cfg.no_applic_k)) {
			switch (keysym) {
			case PK_PF1:
				xkey = 'P';
				break;
	...
	if (PK_ISFKEY(keysym)) {
		...
		if (keysym >= PK_F1 && keysym <= PK_F10 && (modifiers & PKM_SHIFT))
			keysym += 10;
		if ((term->vt52_mode || term->cfg.funky_type == FUNKY_VT100P) && keysym <= PK_F14) {
			...
			if (keysym >= PK_F12)
				offt++;
			p += sprintf((char *) p, term->vt52_mode ? "\x1B%c" : "\x1BO%c", 'P' + keysym - PK_F1 - offt);
			goto done;
		}
	...
	while (s->curr_prompt < p->n_prompts) {
		...
		while (!finished_prompt && inlen) {
			...
			default:
				 ...
				 */
				if ((!pr->echo || (c >= ' ' && c <= '~') || ((unsigned char) c >= 160)) && s->pos < pr->result_len - 1) {
					pr->result[s->pos++] = c;
					if (pr->echo)
