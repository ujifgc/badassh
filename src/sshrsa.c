...
int makekey(unsigned char *data, int len, struct RSAKey *result, unsigned char **keystr, int order) {
...
static Bignum rsa_privkey_op(Bignum input, struct RSAKey *key) {
	...
	while (1) {
		 ...
		 */
		if (bignum_cmp(random, Zero) <= 0 || bignum_cmp(random, key->modulus) >= 0) {
			freebn(random);
			continue;
		} else {
			break;
...
void rsa_fingerprint(char *str, int len, struct RSAKey *key) {
	...
	snprintf(buffer, 16 * 3 + 40, "%d ", bignum_bitcount(key->modulus));
	for (i = 0; i < 16; i++)
		snprintf(buffer + strlen(buffer), 16 * 3 + 40 - strlen(buffer), "%s%02x", i ? ":" : "", digest[i]);
...
static int rsa2_openssh_fmtkey(void *key, unsigned char *blob, int len) {
	...
	bloblen = ssh2_bignum_length(rsa->modulus) + ssh2_bignum_length(rsa->exponent) + ssh2_bignum_length(
			rsa->private_exponent) + ssh2_bignum_length(rsa->iqmp) + ssh2_bignum_length(rsa->p) + ssh2_bignum_length(
			rsa->q);
...
#undef ADD_BIGNUM
	...
	snprintf(buffer, 16 * 3 + 40, "ssh-rsa %d ", bignum_bitcount(rsa->modulus));
	for (i = 0; i < 16; i++)
		snprintf(buffer + strlen(buffer), 16 * 3 + 40 - strlen(buffer), "%s%02x", i ? ":" : "", digest[i]);
 ...
 *    algorithms(2) 26 }
 */
static const unsigned char asn1_weird_stuff[] = { 0x00, 0x30, 0x21, 0x30, 0x09, 0x06, 0x05, 0x2B, 0x0E, 0x03, 0x02,
...
const struct ssh_signkey ssh_rsa = { rsa2_newkey, rsa2_freekey, rsa2_fmtkey, rsa2_public_blob, rsa2_private_blob,
		rsa2_createkey, rsa2_openssh_createkey, rsa2_openssh_fmtkey, rsa2_pubkey_bits, rsa2_fingerprint,
		rsa2_verifysig, rsa2_sign, "ssh-rsa", "rsa2" };
