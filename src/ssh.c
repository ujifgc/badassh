...
static const char * const ssh2_disconnect_reasons[] = { NULL, "host not allowed to connect", "protocol error",
		"key exchange failed", "host authentication failed", "MAC error", "compression error", "service not available",
		"protocol version not supported", "host key not verifiable", "connection lost", "by application",
		"too many connections", "auth cancelled by user", "no more auth methods available", "illegal user name", };
...
static const struct {
	...
	int opcode;
	enum {
		TTY_OP_CHAR, TTY_OP_BOOL
	} type;
} ssh_ttymodes[] = {
/* "V" prefix discarded for special characters relative to SSH specs */
{ "INTR", 1, TTY_OP_CHAR }, { "QUIT", 2, TTY_OP_CHAR }, { "ERASE", 3, TTY_OP_CHAR }, { "KILL", 4, TTY_OP_CHAR }, {
		"EOF", 5, TTY_OP_CHAR }, { "EOL", 6, TTY_OP_CHAR }, { "EOL2", 7, TTY_OP_CHAR }, { "START", 8, TTY_OP_CHAR }, {
		"STOP", 9, TTY_OP_CHAR }, { "SUSP", 10, TTY_OP_CHAR }, { "DSUSP", 11, TTY_OP_CHAR }, { "REPRINT", 12,
		TTY_OP_CHAR }, { "WERASE", 13, TTY_OP_CHAR }, { "LNEXT", 14, TTY_OP_CHAR }, { "FLUSH", 15, TTY_OP_CHAR }, {
		"SWTCH", 16, TTY_OP_CHAR }, { "STATUS", 17, TTY_OP_CHAR }, { "DISCARD", 18, TTY_OP_CHAR }, { "IGNPAR", 30,
		TTY_OP_BOOL }, { "PARMRK", 31, TTY_OP_BOOL }, { "INPCK", 32, TTY_OP_BOOL }, { "ISTRIP", 33, TTY_OP_BOOL }, {
		"INLCR", 34, TTY_OP_BOOL }, { "IGNCR", 35, TTY_OP_BOOL }, { "ICRNL", 36, TTY_OP_BOOL }, { "IUCLC", 37,
		TTY_OP_BOOL }, { "IXON", 38, TTY_OP_BOOL }, { "IXANY", 39, TTY_OP_BOOL }, { "IXOFF", 40, TTY_OP_BOOL }, {
		"IMAXBEL", 41, TTY_OP_BOOL }, { "ISIG", 50, TTY_OP_BOOL }, { "ICANON", 51, TTY_OP_BOOL }, { "XCASE", 52,
		TTY_OP_BOOL }, { "ECHO", 53, TTY_OP_BOOL }, { "ECHOE", 54, TTY_OP_BOOL }, { "ECHOK", 55, TTY_OP_BOOL }, {
		"ECHONL", 56, TTY_OP_BOOL }, { "NOFLSH", 57, TTY_OP_BOOL }, { "TOSTOP", 58, TTY_OP_BOOL }, { "IEXTEN", 59,
		TTY_OP_BOOL }, { "ECHOCTL", 60, TTY_OP_BOOL }, { "ECHOKE", 61, TTY_OP_BOOL }, { "PENDIN", 62, TTY_OP_BOOL }, /* XXX is this a real mode? */
{ "OPOST", 70, TTY_OP_BOOL }, { "OLCUC", 71, TTY_OP_BOOL }, { "ONLCR", 72, TTY_OP_BOOL }, { "OCRNL", 73, TTY_OP_BOOL },
		{ "ONOCR", 74, TTY_OP_BOOL }, { "ONLRET", 75, TTY_OP_BOOL }, { "CS7", 90, TTY_OP_BOOL }, { "CS8", 91,
				TTY_OP_BOOL }, { "PARENB", 92, TTY_OP_BOOL }, { "PARODD", 93, TTY_OP_BOOL } };
...
static unsigned int ssh_tty_parse_boolean(char *s) {
	if (stricmp(s, "yes") == 0 || stricmp(s, "on") == 0 || stricmp(s, "true") == 0 || stricmp(s, "+") == 0)
		return 1; /* true */
	else if (stricmp(s, "no") == 0 || stricmp(s, "off") == 0 || stricmp(s, "false") == 0 || stricmp(s, "-") == 0)
		return 0; /* false */
	else
		return (atoi(s) != 0);
}
...
static int do_ssh1_login(Ssh ssh, unsigned char *in, int inlen, struct Packet *pktin);
...
const static struct ssh_mac *macs[] = { &ssh_hmac_sha1, &ssh_hmac_sha1_96, &ssh_hmac_md5 };
const static struct ssh_mac *buggymacs[] = { &ssh_hmac_sha1_buggy, &ssh_hmac_sha1_96_buggy, &ssh_hmac_md5 };
...
const static struct ssh_compress ssh_comp_none = { "none", ssh_comp_none_init, ssh_comp_none_cleanup,
		ssh_comp_none_block, ssh_comp_none_init, ssh_comp_none_cleanup, ssh_comp_none_block, ssh_comp_none_disable,
		NULL };
extern const struct ssh_compress ssh_zlib;
const static struct ssh_compress *compressions[] = { &ssh_zlib, &ssh_comp_none };

enum { /* channel types */
	CHAN_MAINSESSION, CHAN_X11, CHAN_AGENT, CHAN_SOCKDATA, CHAN_SOCKDATA_DORMANT
...
static int do_ssh2_transport(Ssh ssh, void *vin, int inlen, struct Packet *pktin);
...
struct ssh_tag {
	...
	enum {
		SSH_STATE_PREPACKET, SSH_STATE_BEFORE_SIZE, SSH_STATE_INTERMED, SSH_STATE_SESSION, SSH_STATE_CLOSED
	} state;
...
static void parse_ttymodes(Ssh ssh, char *modes, void(*do_mode)(void *data, char *mode, char *val), void *data) {
	while (*modes) {
		char *t = strchr(modes, '\t');
...
static int s_wrpkt_prepare(Ssh ssh, struct Packet *pkt, int *offset_p) {
	...
	if (ssh->v1_compressing) {
		...
		int complen;
		zlib_compress_block(ssh->cs_comp_ctx, pkt->data + 12, pkt->length - 12, &compblk, &complen);
...
static int ssh2_pkt_construct(Ssh ssh, struct Packet *pkt) {
	 ...
	 */
	{
		...
		int newlen;
		if (ssh->cscomp
				&& ssh->cscomp->compress(ssh->cs_comp_ctx, pkt->data + 5, pkt->length - 5, &newpayload, &newlen)) {
			pkt->length = 5;
...
static void ssh2_pkt_send_noqueue(Ssh ssh, struct Packet *pkt) {
	...
	if (!ssh->kex_in_progress && ssh->max_data_size != 0 && ssh->outgoing_data_size > ssh->max_data_size)
...
static int ssh1_pkt_getrsakey(struct Packet *pkt, struct RSAKey *key, unsigned char **keystr) {
	int j;

	j = makekey(pkt->body + pkt->savedpos, pkt->length - pkt->savedpos, key, keystr, 0);
...
static Bignum ssh1_pkt_getmp(struct Packet *pkt) {
	...
	j = ssh1_read_bignum(pkt->body + pkt->savedpos, pkt->length - pkt->savedpos, &b);
 ...
 */
static void ssh2_add_sigblob(Ssh ssh, struct Packet *pkt, void *pkblob_v, int pkblob_len, void *sigblob_v,
		int sigblob_len) {
	 ...
	 */
	if ((ssh->remote_bugs & BUG_SSH2_RSA_PADDING) && (GET_32BIT(pkblob) == 7 && !memcmp(pkblob + 4, "ssh-rsa", 7))) {
		int pos, len, siglen;
...
static void ssh_detect_bugs(Ssh ssh, char *vstring) {
	 ...
	 */
	if (ssh->cfg.sshbug_ignore1 == FORCE_ON || (ssh->cfg.sshbug_ignore1 == AUTO && (!strcmp(imp, "1.2.18") || !strcmp(
			imp, "1.2.19") || !strcmp(imp, "1.2.20") || !strcmp(imp, "1.2.21") || !strcmp(imp, "1.2.22") || !strcmp(
			imp, "Cisco-1.25") || !strcmp(imp, "OSU_1.4alpha3") || !strcmp(imp, "OSU_1.5alpha4")))) {
		/*
	...
	if (ssh->cfg.sshbug_plainpw1 == FORCE_ON || (ssh->cfg.sshbug_plainpw1 == AUTO && (!strcmp(imp, "Cisco-1.25")
	...
	if (ssh->cfg.sshbug_rsa1 == FORCE_ON || (ssh->cfg.sshbug_rsa1 == AUTO && (!strcmp(imp, "Cisco-1.25")))) {
		/*
	...
	if (ssh->cfg.sshbug_hmac2 == FORCE_ON || (ssh->cfg.sshbug_hmac2 == AUTO && !wc_match("* VShell", imp) && (wc_match(
			"2.1.0*", imp) || wc_match("2.0.*", imp) || wc_match("2.2.0*", imp) || wc_match("2.3.0*", imp) || wc_match(
			"2.1 *", imp)))) {
		/*
	...
	if (ssh->cfg.sshbug_derivekey2 == FORCE_ON || (ssh->cfg.sshbug_derivekey2 == AUTO && !wc_match("* VShell", imp)
	...
	if (ssh->cfg.sshbug_rsapad2 == FORCE_ON || (ssh->cfg.sshbug_rsapad2 == AUTO && (wc_match("OpenSSH_2.[5-9]*", imp)
	...
	if (ssh->cfg.sshbug_pksessid2 == FORCE_ON || (ssh->cfg.sshbug_pksessid2 == AUTO
	...
	if (ssh->cfg.sshbug_rekey2 == FORCE_ON || (ssh->cfg.sshbug_rekey2 == AUTO && (wc_match("DigiSSH_2.0", imp)
			|| wc_match("OpenSSH_2.[0-4]*", imp) || wc_match("OpenSSH_2.5.[0-3]*", imp) || wc_match("Sun_SSH_1.0", imp)
...
static void ssh_process_incoming_data(Ssh ssh, unsigned char **data, int *datalen) {
	struct Packet *pktin;
...
static void ssh_log(Plug plug, int type, SockAddr addr, int port, const char *error_msg, int error_code) {
	Ssh ssh = (Ssh) plug;
...
static int ssh_closing(Plug plug, const char *error_msg, int error_code, int calling_back) {
	Ssh ssh = (Ssh) plug;
...
/*
 * Dns resolved an address and wants allows ssh to connect
 */
static void ssh_dns(Plug plug, SockAddr addr) {
	Ssh ssh = (Ssh) plug;
	if (addr) {
		ssh->s = new_connection(addr, 0, ssh->cfg.port, 0, 1, 1, 1, (Plug) ssh, &ssh->cfg);
	}else{
		logerror("NS lookup failed: %s\n", ssh->cfg.host);
		notify_remote_exit(ssh->frontend);
	}
}
 ...
 * freed by the caller.
 */
static const char *connect_to_host(Ssh ssh, const char *host) {
	static const struct plug_function_table fn_table = { ssh_log, ssh_closing, ssh_receive, ssh_sent, NULL, ssh_dns };

	ssh->savedhost = snewn(1 + strlen(host), char);
	strcpy(ssh->savedhost, host);

	ssh->savedport = ssh->cfg.port;
	ssh->fn = &fn_table;

	name_lookup(host, (Plug) ssh);

	/*
	 * ssh_dns on dns respponse
	 */
 ...
 * => log `wire_reason'.
 */
static void ssh_disconnect(Ssh ssh, char *client_reason, char *wire_reason, int code, int clean_exit) {
	char *error;
	if (!client_reason)
...
static void ssh1_smsg_stdout_stderr_data(Ssh ssh, struct Packet *pktin) {
	...
	bufsize = from_backend(ssh->frontend, pktin->type == SSH1_SMSG_STDERR_DATA, string, stringlen);
...
static void ssh2_mkkey(Ssh ssh, Bignum K, unsigned char *H, char chr, unsigned char *keyspace) {
...
static void ssh2_msg_channel_data(Ssh ssh, struct Packet *pktin) {
	...
	if (pktin->type == SSH2_MSG_CHANNEL_EXTENDED_DATA && ssh_pkt_getuint32(pktin) != SSH2_EXTENDED_DATA_STDERR)
	...
	if (data) {
		...
		switch (c->type) {
		case CHAN_MAINSESSION:
			bufsize = from_backend(ssh->frontend, pktin->type == SSH2_MSG_CHANNEL_EXTENDED_DATA, data, length);
			break;
		}
		/*
...
static void ssh2_msg_channel_open_failure(Ssh ssh, struct Packet *pktin) {
	static const char * const reasons[] = { "<unknown reason code>", "Administratively prohibited", "Connect failed",
	...
	logeventf(ssh, "Forwarded connection refused by server: %s [%.*s]", reasons[reason_code], reason_length,
			reason_string);
...
#undef TRANSLATE_SIGNAL
			...
			logeventf(ssh, "Server exited on signal%s%s%s", fmt_sig, core ? " (core dumped)" : "", fmt_msg);
			if (*fmt_sig)
				sfree(fmt_sig);
			if (*fmt_msg)
				sfree(fmt_msg);
...
static void ssh2_msg_channel_open(Ssh ssh, struct Packet *pktin) {
	...
	if (typelen == 22 && !memcmp(type, "auth-agent@openssh.com", 22)) {
...
static void ssh2_msg_disconnect(Ssh ssh, struct Packet *pktin) {
	...
	if (reason > 0 && reason < lenof(ssh2_disconnect_reasons)) {
		buf = dupprintf("Received disconnect message (%s)", ssh2_disconnect_reasons[reason]);
	} else {
		buf = dupprintf("Received disconnect message (unknown type %d)", reason);
	}
	logevent(buf);
	sfree(buf);
	notify_remote_exit(ssh->frontend);
...
static void ssh2_timer(void *ctx, long now) {
	...
	if (!ssh->kex_in_progress && ssh->cfg.ssh_rekey_time != 0 && now - ssh->next_rekey >= 0) {
...
static void ssh2_protocol(Ssh ssh, void *vin, int inlen, struct Packet *pktin) {
	...
	if (pktin) {
		...
		if (!ssh->kex_in_progress && ssh->max_data_size != 0 && ssh->incoming_data_size > ssh->max_data_size)
	...
	if (!ssh->protocol_initial_phase_done || (pktin && pktin->type >= 20 && pktin->type < 50)) {
		if (do_ssh2_transport(ssh, in, inlen, pktin) && !ssh->protocol_initial_phase_done) {
			ssh->protocol_initial_phase_done = TRUE;
			/*
 ...
 * Returns an error message, or NULL on success.
 */
static const char *ssh_init(void *frontend_handle, void **backend_handle, Config *cfg, const char *host) {
	const char *p;
	Ssh ssh;
	...
	*backend_handle = ssh;
	...
	ssh->incoming_data_size = ssh->outgoing_data_size = ssh->deferred_data_size = 0L;
	ssh->max_data_size = parse_blocksize(ssh->cfg.ssh_rekey_data);
	ssh->kex_in_progress = FALSE;

	p = connect_to_host(ssh, host);
	if (p != NULL)
		return p;
...
static void ssh_reconfig(void *handle, Config *cfg) {
	...
	if (ssh->cfg.ssh_rekey_time != cfg->ssh_rekey_time && cfg->ssh_rekey_time != 0) {
	...
	if (old_max_data_size != ssh->max_data_size && ssh->max_data_size != 0) {
		if (ssh->outgoing_data_size > ssh->max_data_size || ssh->incoming_data_size > ssh->max_data_size)
	...
	if (ssh->cfg.ssh2_des_cbc != cfg->ssh2_des_cbc || memcmp(ssh->cfg.ssh_cipherlist, cfg->ssh_cipherlist,
...
static void ssh_size(void *handle, int width, int height) {
	...
	case SSH_STATE_SESSION:
		if (!ssh->cfg.nopty) {
			if (ssh->version == 1) {
				send_packet(ssh, SSH1_CMSG_WINDOW_SIZE, PKT_INT, ssh->term_height, PKT_INT, ssh->term_width, PKT_INT,
						0, PKT_INT, 0, PKT_END);
			} else if (ssh->mainchan) {
...
static const struct telnet_special *ssh_get_specials(void *handle) {
	static const struct telnet_special ssh1_ignore_special[] = { { "IGNORE message", TS_NOP } };
	static const struct telnet_special ssh2_transport_specials[] = { { "IGNORE message", TS_NOP }, {
			"Repeat key exchange", TS_REKEY }, };
	static const struct telnet_special ssh2_session_specials[] = { { NULL, TS_SEP }, { "Break", TS_BRK },
	 ...
	 * required signals. */
	{ "SIGINT (Interrupt)", TS_SIGINT }, { "SIGTERM (Terminate)", TS_SIGTERM }, { "SIGKILL (Kill)", TS_SIGKILL }, {
			"SIGQUIT (Quit)", TS_SIGQUIT }, { "SIGHUP (Hangup)", TS_SIGHUP }, { "More signals", TS_SUBMENU }, {
			"SIGABRT", TS_SIGABRT }, { "SIGALRM", TS_SIGALRM }, { "SIGFPE", TS_SIGFPE }, { "SIGILL", TS_SIGILL }, {
			"SIGPIPE", TS_SIGPIPE }, { "SIGSEGV", TS_SIGSEGV }, { "SIGUSR1", TS_SIGUSR1 }, { "SIGUSR2", TS_SIGUSR2 }, {
			NULL, TS_EXITMENU } };
	static const struct telnet_special specials_end[] = { { NULL, TS_EXITMENU } };
	/* XXX review this length for any changes: */
	static struct telnet_special ssh_specials[lenof(ssh2_transport_specials) + lenof(ssh2_session_specials)
			+ lenof(specials_end)];
	Ssh ssh = (Ssh) handle;
	int i = 0;
...
void ssh_send_port_open(void *channel, char *hostname, int port, char *org) {
	...
	if (ssh->version == 1) {
		send_packet(ssh, SSH1_MSG_PORT_OPEN, PKT_INT, c->localid, PKT_STR, hostname, PKT_INT, port,
		/* PKT_STR, <org:orgport>, */
		PKT_END);
	} else {
...
Backend ssh_backend = { ssh_init, ssh_free, ssh_reconfig, ssh_send, ssh_sendbuffer, ssh_size, ssh_special,
		ssh_get_specials, ssh_connected, ssh_return_exitcode, ssh_sendok, ssh_ldisc, ssh_provide_ldisc,
		NULL, ssh_unthrottle, ssh_cfg_info, 22 };
