...
static int loadrsakey_main(FILE * fp, struct RSAKey *key, int pub_only, char **commentptr, char *passphrase,
		const char **error) {
	...
	*error = "file format error";
	...
	end: memset(buf, 0, sizeof(buf)); /* burn the evidence */
	return ret;
}

int loadrsakey(const Filename *filename, struct RSAKey *key, char *passphrase, const char **errorstr) {
	FILE *fp;
	char buf[64];
	int ret = 0;
 ...
 * exponent, modulus).
 */
int rsakey_pubblob(const Filename *filename, void **blob, int *bloblen, char **commentptr, const char **errorstr) {
	FILE *fp;
	char buf[64];
	struct RSAKey key;
	int ret;
...
struct ssh2_userkey *ssh2_load_userkey(const Filename *filename, char *passphrase, const char **errorstr) {
	FILE *fp;
	...
	if (!read_header(fp, header))
		goto error;
	if (strcmp(header, "PuTTY-User-Key-File-2")) {
	 ...
	 */
	{
...

		{
			...
			maclen = (4 + namelen + 4 + enclen + 4 + commlen + 4 + public_blob_len + 4 + private_blob_len);
...
#define DO_STR(s,len) PUT_32BIT(p,(len));memcpy(p+4,(s),(len));p+=4+(len)
		...
		for (i = 0; i < 20; i++)
			snprintf(realmac + 2 * i, 41 - 2 * i, "%02x", binary[i]);
	...
	ret->alg = alg;
	ret->comment = comment;
	ret->data = alg->createkey(public_blob, public_blob_len, private_blob, private_blob_len);
	if (!ret->data) {
		sfree(ret->comment);
		sfree(ret);
		ret = NULL;
...
unsigned char *ssh2_userkey_loadpub(const Filename *filename, char **algorithm, int *pub_blob_len, char **commentptr,
		const char **errorstr) {
	FILE *fp;
	char header[40], *b;
	const struct ssh_signkey *alg;
	...
	/* Read the first header line which contains the key type. */
	if (!read_header(fp, header) || (0 != strcmp(header, "PuTTY-User-Key-File-2") && 0 != strcmp(header,
...
int ssh2_save_userkey(const Filename *filename, struct ssh2_userkey *key, char *passphrase) {
	FILE *fp;
	...
	memcpy(priv_blob_encrypted + priv_blob_len, priv_mac, priv_encrypted_len - priv_blob_len);
	...
	/* Now create the MAC. */
	{
		...
		maclen = (4 + namelen + 4 + enclen + 4 + commlen + 4 + pub_blob_len + 4 + priv_encrypted_len);
		macdata = snewn(maclen, unsigned char);
		p = macdata;
#define DO_STR(s,len) PUT_32BIT(p,(len));memcpy(p+4,(s),(len));p+=4+(len)
